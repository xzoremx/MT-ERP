...
if (isset($_GET["id"])) {
    ...
    // Si no se envió el formulario POST, obtén los datos existentes de la cotización
    if ($_SERVER["REQUEST_METHOD"] !== "POST") {
        $sql = "SELECT fecha, ID_Cliente, `Nombre del Cliente`, Servicio, Producto, Cargo, ID_Responsable, Estado, fechasistema FROM ordenes_de_cotizacion WHERE ID_Cotizacion = ?";
        $stmt = $mysqli->prepare($sql);

        if ($stmt) {
            $stmt->bind_param("i", $id_cotizacion);
            $stmt->execute();
            $stmt->bind_result($fecha, $ID_Cliente, $Nombre_del_Cliente, $Servicio, $Producto, $Cargo, $ID_Responsable, $Estado, $fechasistema);
            $stmt->fetch();
            $stmt->close();
        }
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['fecha'], $_POST['ID_Cliente'], $_POST['Nombre_del_Cliente'], $_POST['Servicio'], $_POST['Producto'], $_POST['Cargo'], $_POST['ID_Responsable'], $_POST['Estado'])) {
        // Procesa los datos del formulario de edición y actualiza la cotización en la base de datos
        $nuevos_datos = $_POST;

        // Realiza la actualización en la base de datos utilizando la variable $id_cotizacion como referencia
        $sql = "UPDATE ordenes_de_cotizacion SET fecha=?, ID_Cliente=?, `Nombre del Cliente`=?, Servicio=?, Producto=?, Cargo=?, ID_Responsable=?, Estado=?, fechasistema=? WHERE ID_Cotizacion=?";
        $stmt = $mysqli->prepare($sql);

        if ($stmt) {
            $stmt->bind_param("sssssssssi", $nuevos_datos['fecha'], $nuevos_datos['ID_Cliente'], $nuevos_datos['Nombre_del_Cliente'], $nuevos_datos['Servicio'], $nuevos_datos['Producto'], $nuevos_datos['Cargo'], $nuevos_datos['ID_Responsable'], $nuevos_datos['Estado'], $fechasistema, $id_cotizacion);
    ...
    <section class="content">
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    <button id="submit" name="button" value="submit" class="btn btn-primary">Consultar</button>
                                        ...
                                        <a class="btn btn-success btn-sm" href="COMVE-011-Agregar.php">
        ...
        th {
            ...
            color: black; /* Color de texto blanco para los encabezados */
                    ...
                    <title>Editar Orden de Cotización</title>
</head>
<body>
    ...
    <form method="post">
        <table>
            ...
            <tbody>
                <tr>
                    <td><?php echo htmlspecialchars($id_cotizacion); ?></td>
                    ...
                    <td><input type="text" name="Servicio" value="<?php echo htmlspecialchars($Servicio); ?>" maxlength="40"></td>
                    <td><input type="text" name="Producto" value="<?php echo htmlspecialchars($Producto); ?>" maxlength="40"></td>
                    <td><input type="text" name="Cargo" value="<?php echo htmlspecialchars($Cargo); ?>" maxlength="10"></td>
                    <td><input type="text" name="ID_Responsable" value="<?php echo htmlspecialchars($ID_Responsable); ?>" maxlength="10"></td>
