...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "INSERT INTO registro_venta (ID_Cotizacion, ID_Responsable, fecha, Tipo_Comprobante, fechasistema, Servicio, Producto, ID_Cliente, Estado, Modalidad_Pago, Monto_total) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($sql);

    if ($stmt) {
    $stmt->bind_param("sssssssssss", $nuevos_datos['ID_Cotizacion'], $nuevos_datos['ID_Responsable'], $nuevos_datos['fecha'], $nuevos_datos['Tipo_Comprobante'], $nuevos_datos['fechasistema'], $nuevos_datos['Servicio'], $nuevos_datos['Producto'], $nuevos_datos['ID_Cliente'], $nuevos_datos['Estado'], $nuevos_datos['Modalidad_Pago'], $nuevos_datos['Monto_total']);
    // Resto del código para ejecutar la inserción
        if ($stmt->execute()) {
            // Redirige de nuevo a la página principal u otra página de tu elección
            header("Location: COMVE-004.php");
            exit();
        } else {
...
<div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <!-- Tabla de Registro del Ventas con estilo -->
    ...
    <form method="post">
        <table id="listado" class="table table-bordered table-striped">
            ...
            <tbody>
                <tr>
                    <td>Nuevo</td>
                    <td><input type="text" name="ID_Cotizacion"></td>
                    <td><input type="text" name="ID_Responsable"></td>
                    <td><input type="text" name="fecha" class="datepicker"></td>
                    <td><input type="text" name="Tipo_Comprobante"></td>
                    <td><?php echo date("Y-m-d H:i:s"); ?></td>
                    <td><input type="text" name="Servicio"></td>
                    <td><input type="text" name="Producto"></td>
                    <td><input type="text" name="ID_Cliente"></td>
                    <td><input type="text" name="Estado"></td>
                    <td><input type="text" name="Modalidad_Pago"></td>
                    <td><input type="text" name="Monto_total"></td>

                    <td>
                        <input type="button" value="Agregar Registro de Venta" onclick="confirmarRegistro()">
                    </td>
                    <div id="confirmacion" style="display: none;">
                        <p>¿Desea realmente agregar este Registro?</p>
                        <button onclick="registrarVenta()">Sí</button>
                        <button onclick="cancelarRegistro()">No</button>
                    </div>
                    <div id="exito" style="display: none;">
                        <p>Venta registrado con éxito.</p>
                    </div>
                </tr>
            </tbody>
        </table>
