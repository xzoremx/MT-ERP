...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "INSERT INTO descuentos_ofertas (Nombre_Oferta, Descripcion, Tipo_Oferta, Servicio, fechasistema, Fecha_inicio, Fecha_expiracion, Estado, Valor_Descuento) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($sql);

    if ($stmt) {
    $stmt->bind_param("sssssssss", $nuevos_datos['Nombre_Oferta'], $nuevos_datos['Descripcion'], $nuevos_datos['Tipo_Oferta'], $nuevos_datos['Servicio'], $nuevos_datos['fechasistema'], $nuevos_datos['Fecha_inicio'], $nuevos_datos['Fecha_expiracion'], $nuevos_datos['Estado'], $nuevos_datos['Valor_Descuento']);
    // Luego, ejecuta la consulta y realiza cualquier otra acción necesaria.
        if ($stmt->execute()) {
            // Redirige de nuevo a la página principal u otra página de tu elección
            header("Location: COMVE-003.php");
            exit();
        } else {
...
<div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <!-- Tabla de Registro del Ofertas con estilo -->
                <h1>Registro de Descuentos y Ofertas</h1>
    ...
    <form method="post">
        <table id="listado" class="table table-bordered table-striped">
            ...
            <tbody>
                <tr>
                <td>Nuevo</td>
                <td><input type="text" name="Nombre_Oferta"></td>
                <td><input type="text" name="Descripcion"></td>
                <td><input type="text" name="Tipo_Oferta"></td>
                <td><input type="text" name="Servicio"></td>
                <td><?php echo date("Y-m-d H:i:s"); ?></td>
                <td><input type="text" name="Fecha_inicio" class="datepicker"></td>
                <td><input type="text" name="Fecha_expiracion" class="datepicker"></td>
                <td><input type="text" name="Estado"></td>
                <td><input type="text" name="Valor_Descuento"></td>
                <td>
                    <input type="button" value="Agregar Oferta" onclick="confirmarRegistro()">
                </td>
                <div id="confirmacion" style="display: none;">
                    <p>¿Desea realmente agregar esta Oferta?</p>
                    <button onclick="registrarOferta()">Sí</button>
                    <button onclick="cancelarRegistro()">No</button>
                </div>
                <div id="exito" style="display: none;">
                    <p>Oferta registrada con éxito.</p>
                </div>
                </tr>
            </tbody>
        </table>
