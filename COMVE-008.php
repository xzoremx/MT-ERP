...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "INSERT INTO encuesta (Nombre_Encuesta, Tipo_Encuesta, Link_Encuesta, Canal_Distribucion, fechasistema, Activacion_Alertas, Fecha_Envio, Fecha_Cierre, Comentario, Estado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($sql);

    if ($stmt) {
    $stmt->bind_param("ssssssssss", $nombreEncuesta, $tipoEncuesta, $linkEncuesta, $canalDistribucion, $fechaSistema, $activacionAlertas, $fechaEnvio, $fechaCierre, $comentario, $estado);

        if ($stmt->execute()) {
            // Redirige de nuevo a la página principal u otra página de tu elección
            header("Location: COMVE-008.php");
            exit();
        } else {
...
<div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <!-- Tabla con estilo -->
                <h1>Recopilación de Retroalimentaciones de Clientes</h1>
    ...
    <form method="post">
        <table id="listado" class="table table-bordered table-striped">
            ...
            <tbody>
                <tr>
                <td>Nuevo</td> <!-- El ID_Encuesta se genera automáticamente -->
                <td><input type="text" name="Nombre_Encuesta"></td>
                <td><input type="text" name="Tipo_Encuesta"></td>
                <td><input type="text" name="Link_Encuesta"></td>
                <td><input type="text" name="Canal_Distribucion"></td>
                <td><?php echo date("Y-m-d H:i:s"); ?></td> <!-- Fecha actual -->
                <td><input type="text" name="Activacion_Alertas"></td>
                <td><input type="text" name="Fecha_Envio" class="datepicker"></td>
                <td><input type="text" name="Fecha_Cierre" class="datepicker"></td>
                <td><input type="text" name="Comentario"></td>
                <td><input type="text" name="Estado"></td>
                    <td>
                        <input type="button" value="Agregar Encuesta" onclick="confirmarRegistro()">
                    </td>
                    <div id="confirmacion" style="display: none;">
                        <p>¿Desea realmente agregar esta encuesta?</p>
                        <button onclick="registrarEncuesta()">Sí</button>
