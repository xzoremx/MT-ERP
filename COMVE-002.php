...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "INSERT INTO cotizacion_servicio (ID_Cliente, ID_Responsable, Servicio, `Dirección de Partida`, fechasistema, `Dirección de Llegada`, `Fecha Límite`, Descuento, Estado, `Monto Total`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($sql);

    if ($stmt) {
    $stmt->bind_param("ssssssssss", $nuevos_datos['ID_Cliente'], $nuevos_datos['ID_Responsable'], $nuevos_datos['Servicio'], $nuevos_datos['Dirección de Partida'], $nuevos_datos['fechasistema'], $nuevos_datos['Dirección de Llegada'], $nuevos_datos['Fecha Límite'], $nuevos_datos['Descuento'], $nuevos_datos['Estado'], $nuevos_datos['Monto Total']);
    // Resto del código para ejecutar la consulta y registrar los datos.
        if ($stmt->execute()) {
            // Redirige de nuevo a la página principal u otra página de tu elección
            header("Location: COMVE-002.php");
            exit();
        } else {
...
<div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <!-- Tabla de Generación de Cotizaciones de Servicio con estilo -->
                <h1>Generación de Cotizaciones de Servicio</h1>
    ...
    <form method="post">
        <table id="listado" class="table table-bordered table-striped">
            <thead>
                ...
                <th>fechasistema</th> <!-- Esto se mantiene como fechasistema. -->
            ...
            <tbody>
                ...
                <td><input type="text" name="direccion_partida"></td>
                <td><?php echo date("Y-m-d H:i:s"); ?></td>
                <td><input type="text" name="direccion_llegada"></td>
                <td><input type="text" name="fecha_limite" class="datepicker"></td>
                <td><input type="text" name="descuento"></td>
                <td><input type="text" name="estado"></td>
                <td><input type="text" name="monto_total"></td>
                <td>
                    <input type="button" value="Agregar Cotizacion" onclick="confirmarRegistro()">
                </td>
                <div id="confirmacion" style="display: none;">
                    <p>¿Desea realmente agregar este cotizacion?</p>
                    <button onclick="registrarCServicio()">Sí</button>
                    <button onclick="cancelarRegistro()">No</button>
                </div>
                <div id="exito" style="display: none;">
                    <p>Cotizacion registrada con éxito.</p>
                </div>
